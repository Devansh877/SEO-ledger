...
npm run doctor   # confirm nothing broke after the migration
```
...
npm run seed               # creates the real admin login + NexIT as a client
npm run dev                 # http://localhost:4000
```

`npm run seed` prints the admin password (and NexIT's client-login password)
to the terminal **once** — copy it immediately, it's hashed before storage
and can't be retrieved again afterward. Re-running `seed` is safe; it won't
touch an admin or client that already exists.
...
### Diagnosing what's actually connected
```
cd backend
npm run doctor
```
Makes real API calls (no writes) and reports, per client and per source,
whether GA4/Search Console are genuinely connected or silently falling back
to mock data — including listing every Search Console property the service
account can actually see, which is usually where a misconfigured
`gscSiteUrl` shows up. Exits 1 if anything configured is actually failing,
so it's safe to wire into a deploy check.

### Removing demo/test data
If you'd previously seeded this with demo clients (Cyberforte, Meridian
Dental, Alderton & Co) from an earlier version of this project:
```
npm run purge:demo              # dry run — lists exactly what it would delete
npm run purge:demo -- --confirm # actually deletes
```
Matches only those three exact client names and the old
`admin@nexit.demo` / `*@client.demo` logins — it cannot catch a real client
by accident, and does nothing until you pass `--confirm`.

## Real data sources — what's wired up and what's still mock

| Module | Service file | Status |
|---|---|---|
| GA4-01 | `backend/src/services/ga4.service.js` | **Real** — GA4 Data API via a service account. Falls back to mock if `GA4_SERVICE_ACCOUNT_KEY_BASE64` isn't set, or if `client.ga4PropertyId` is blank, or if the real call errors (logged, not silently swallowed) |
| KWD-02 (search_console source) | `backend/src/services/gsc.service.js` | **Real** — Search Console `searchanalytics.query`, same service account. Falls back to mock the same way; if the real call succeeds but finds zero impressions for a keyword, no snapshot is written that round rather than fabricating a number |
| KWD-02 (dataforseo source) | `backend/src/services/keywords.service.js` | Still mock — swap `callDataForSeo()` for a real DataForSEO SERP + Keywords Data API call |
| GMB-03 | `backend/src/services/gmb.service.js` | Still mock — swap for the Google Business Profile Performance + Business Information APIs |
| CNV-04 | `backend/src/services/conversions.service.js` | Still mock — reuses the same GA4 Data API client as GA4-01, filtered to key events |

### Setting up the real GA4 / Search Console connection
1. One Google Cloud project, one service account, both APIs enabled on it
   (`analyticsdata.googleapis.com` and `searchconsole.googleapis.com`) —
   see `backend/src/lib/googleAuth.js` for the shared auth helper both
   services use.
2. Base64-encode the service account's JSON key and set it as
   `GA4_SERVICE_ACCOUNT_KEY_BASE64` (both service files check this same
   variable, with `GSC_SERVICE_ACCOUNT_KEY_BASE64` as a fallback in case you
   ever split them across two service accounts).
3. **Per client**, not once for the whole platform: add the service
   account's email as a Viewer on that client's actual GA4 property, and as
   a user on their Search Console property.
4. Set that client's `ga4PropertyId` (e.g. `properties/123456789`) and
   `gscSiteUrl` (e.g. `https://example.com.au/`, or `sc-domain:example.com.au`
   for a domain property) from the client detail page in the admin UI
   (Integration Settings) — until these are filled in, that client's
   captures keep using mock data automatically, no code change needed.

### Note on rank history in general
None of DataForSEO, Search Console, or any SERP API return trend data for
free — each only ever answers "what's the position right now" (or, for
Search Console, "what's the average over this date range right now"). That's
why this app never queries any of them live from a dashboard load — the
weekly cron writes a `RankSnapshot` row per keyword per source, and the
dashboard's "previous vs current" and sparklines are entirely a function of
that stored history, not something any single API call can answer on its
own.
