...
export default function ClientDetailPage() {
  ...
  async function handleDelete() {
    const sure = window.confirm(
      `Delete "${client.name}" permanently? This removes their login, access ledger, and all captured history. This cannot be undone.`
    );
    if (!sure) return;
        ...
        <div className="max-w-[1080px] mx-auto px-8 py-7 w-full">
          ...
          <div className="flex items-start justify-between gap-4 mb-1">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-medium text-red border border-red/30 rounded px-2.5 py-1.5 hover:bg-red/5 shrink-0"
